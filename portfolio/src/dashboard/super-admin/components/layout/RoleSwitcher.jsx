  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100:bg-slate-800 transition-colors text-xs"
      >
        <Shield className="w-3.5 h-3.5 text-primary" />
        <span className="font-semibold text-slate-800">{currentRole.label}</span>
        <span className="text-slate-400">|</span>
        <span className="text-slate-500 truncate max-w-[120px] hidden sm:inline">
          {activeClinic.name}
        </span>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-72 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 p-2 space-y-3 animate-in fade-in slide-in-from-top-2 duration-150">
          {/* Role Selection */}
          <div>
            ...
            <div className="mt-1 space-y-0.5">
              {clinicsScope.map((clinic) => (
                ...
                >
                  ...
                  {activeClinic.id === clinic.id && <Check className="w-3.5 h-3.5 text-slate-900" />}
                </button>
              ))}
            </div>
          </div>
