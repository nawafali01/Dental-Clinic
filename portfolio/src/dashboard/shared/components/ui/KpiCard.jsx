...
export const KpiCard = ({ title, value, change, isPositive, period, category, sparkline, icon: Icon }) => {
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-2xs hover:shadow-md transition-all group flex flex-col justify-between">
      <div>
        {/* Top Meta Row */}
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
        ...
        <span className="text-[11px] text-slate-400 truncate">{period}</span>
      </div>
    </div>
  );
};
