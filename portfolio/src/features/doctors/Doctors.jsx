...
import { Globe, ExternalLink, Share2, CalendarPlus } from "lucide-react";
    ...
    const fetchDoctors = async () => {
      try {
        ...
        const normalizedDoctors = items.map((doctor) => {
          const fileName = doctor.img?.split("/").pop();
          return {
            ...doctor,
            img: doctorAssetMap[fileName] ?? doctor.img,
          };
        });
        setDoctors(normalizedDoctors);
      } catch (error) {
        console.error("Could not load doctors from backend", error);
        setDoctors([]);
      }
    };
    fetchDoctors();
